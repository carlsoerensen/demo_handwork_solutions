      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden chat-container">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 md:space-x-3">
              ...
              <div className="min-w-0 flex-1">
                <h2 className="text-base md:text-lg font-bold truncate">Intern AI Agent</h2>
                <p className="text-blue-100 text-xs">Administrativ assistent</p>
              </div>
            </div>
            <button 
              onClick={handleRefresh}
              className="p-1.5 md:p-2 hover:bg-blue-800 rounded-lg transition-colors flex-shrink-0"
              title="Genstart chat"
            >
        ...
        <div className="flex flex-col h-[400px] md:h-[450px]">
          ...
          <div className="p-2 md:p-4 border-b bg-gray-50">
            <p className="text-xs md:text-sm font-medium text-gray-700 mb-2 md:mb-3">Hurtige handlinger:</p>
            <div className="grid grid-cols-3 md:grid-cols-5 gap-1 md:gap-3">
              {quickActions.map((action, index) => (
                <button
                  key={index}
                  onClick={() => setInput(action.action)}
                  className="flex flex-col items-center space-y-1 px-1 md:px-4 py-1.5 md:py-3 bg-white border border-gray-200 rounded-lg hover:bg-blue-50 hover:border-blue-300 hover:shadow-md transition-all duration-200 text-xs font-medium"
                  title={action.action}
                >
                  <div className="text-blue-600 w-3 h-3 md:w-4 md:h-4">
                    {action.icon}
                  </div>
                  <span className="text-gray-700 text-xs text-center leading-tight hidden md:block">{action.label}</span>
                  <span className="text-gray-700 text-xs text-center leading-tight md:hidden">{action.label.split(' ')[0]}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2 md:space-y-3">
            {messages.map((message) => (
              ...
              >
                <div
                  className={`max-w-[85%] sm:max-w-xs lg:max-w-md px-3 md:px-4 py-2 md:py-3 rounded-lg shadow-sm ${
                    message.role === 'user'
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white text-gray-800 px-3 md:px-4 py-2 md:py-3 rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="p-3 md:p-4 border-t bg-gray-50">
            <form onSubmit={handleSubmit} className="flex space-x-2">
              ...
              <button
                ...
                className="bg-blue-600 text-white p-2 md:p-3 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md flex-shrink-0"
                title="Send besked"
              >
